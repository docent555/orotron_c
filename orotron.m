...
%     OUTFvsZ = zeros(size(OUTF,1), 2*size(OUTF,2));
%     OUTJvsZ = zeros(size(OUTJ,1), 2*size(OUTJ,2));
%     OUTFvsZ(:,1:2:end-1) = real(OUTF);
%     OUTFvsZ(:,2:2:end) = imag(OUTF);
%     OUTJvsZ(:,1:2:end-1) = real(OUTJ);
%     OUTJvsZ(:,2:2:end) = imag(OUTJ);
%     
%     OUTF = OUTF.';
%     OUTJ = OUTJ.';
%     OUTFvsT = zeros(size(OUTF,1), 2*size(OUTF,2));
%     OUTJvsT = zeros(size(OUTJ,1), 2*size(OUTJ,2));
%     OUTFvsT(:,1:2:end-1) = real(OUTF);
%     OUTFvsT(:,2:2:end) = imag(OUTF);
%     OUTJvsT(:,1:2:end-1) = real(OUTJ);
%     OUTJvsT(:,2:2:end) = imag(OUTJ);      
%     
%     OUTFvsZ = [OUTZAxis OUTFvsZ];
%     OUTJvsZ = [OUTZAxis OUTJvsZ];
%     
%     OUTFvsT = [OUTTAxis OUTFvsT];
%     OUTJvsT = [OUTTAxis OUTJvsT];
    
%     fileParameters = sprintf('%s', 'parameters.txt');    
%     fileResultsBvsZ = sprintf('%s', 'resultsBvsZ.dat');
%     fileResultsJvsZ = sprintf('%s', 'resultsJvsZ.dat');
%     fileResultsBvsT = sprintf('%s', 'resultsBvsT.dat');
%     fileResultsJvsT = sprintf('%s', 'resultsJvsT.dat');
%     fileT = sprintf('%s', 'Time.dat');
%     fileZ = sprintf('%s', 'Z.dat');
%     
%     strParam = 'Ne = %d\nTend = %f\nLz = %f\nDelta = %f\nI = %f\ndz = %f\ndt = %f\n';
%     fileID = fopen(fileParameters ,'wt');
%     fprintf(fileID, strParam, int64(Ne), Tend, Lz, Delta, Ic, dz, dt);
%     fclose(fileID);    
%     save(fileResultsBvsZ, 'OUTFvsZ', '-ascii');
%     save(fileResultsJvsZ, 'OUTJvsZ', '-ascii');
%     save(fileResultsBvsT, 'OUTFvsT', '-ascii');
